...
def is_motion_query(query: str) -> bool:
    q = query.lower()
    # 1. Temporal transition indicators
    temporal_words = [
        'first', 'then', 'after', 'before', 'while', 'during', 
        'next', 'second', 'start', 'begin', 'end', 'finally'
    ]
    if any(w in q for w in temporal_words):
        return True
        
    # 2. Action verbs and movement keywords
    motion_verbs = [
        'run', 'walk', 'swim', 'fly', 'jump', 'hop', 'crawl', 'move', 'turn', 
        'sit', 'stand', 'chase', 'follow', 'slide', 'roll', 'spin', 'rotate', 
        'dance', 'play', 'climb', 'fall', 'rise', 'exit', 'enter', 'appear', 
        'disappear', 'stop', 'halt', 'accelerate', 'decelerate', 'shake', 'wave'
    ]
    if any(verb in q for verb in motion_verbs):
        return True
        
    return False


def crop_tube_sequence(video_frames, masks, target_size=(224, 224)):
    """
    Crops the tracked object's bounding box across all frames 
    and creates a temporal video clip for X-CLIP.
    video_frames: List of torch.Tensor or numpy arrays of shape [H, W, C]
    masks: List or tensor of shape [T, H, W]
    """
    import cv2
    import numpy as np
    import torch
    
    # Convert masks to numpy if torch tensor
    ...
    for t in range(T):
        ...
        if isinstance(frame, torch.Tensor):
            ...
            # If tensor is normalized or in [0, 1], scale to [0, 255]
            if frame.max() <= 1.0:
    ...
    return np.stack(crops, axis=0) # shape [T, H, W, 3]
