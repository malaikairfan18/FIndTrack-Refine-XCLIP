...
def test(args):
    ...
    for idx_, video in enumerate(video_list):
        ...
        for e in range(num_expressions):
            ...
            if is_motion:
                print(f"  Exp: '{exp}' [Motion detected. Running X-CLIP pathway]")
                for ref_idx, i in enumerate(candidate_indices):
                ...
                for idx in top_2_indices:
                    ...
                    # Forward pass
                    for i in range(f_idx, video_len):
                        if i == f_idx:
                            mask_prob = processor.step(imgs_cutie[i].cuda(), ref_masks[idx].squeeze(0), objects=[1])
                        else:
                            mask_prob = processor.step(imgs_cutie[i].cuda())
                        mask = processor.output_prob_to_mask(mask_prob).float()
                        if i == video_len - 1:
                            processor.clear_memory()
                        tube_masks[i] = mask.detach().cpu().numpy().squeeze(0)
